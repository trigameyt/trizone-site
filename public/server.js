...
app.post('/api/minecraft/duels/snapshot', requireMinecraftSecret, minecraftSyncLimiter, async (req, res) => {
  ...
  try {
    ...
    if (authoritative && incomingKitKeys.length) {
...
app.get('/api/duels/kits', async (_req,res) => {
  try { const r=await query('SELECT kit_key AS key, display_name AS name, icon_material AS icon, emoji, sort_order FROM duel_kits WHERE active=TRUE ORDER BY sort_order, display_name'); res.json({kits:r.rows, tiers:duelTierThresholds}); }
